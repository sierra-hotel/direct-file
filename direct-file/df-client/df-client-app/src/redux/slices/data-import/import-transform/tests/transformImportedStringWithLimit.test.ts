...
describe(`transformImportedStringWithLimit`, () => {
  describe(`for about you basic data`, () => {
    ...
    it(`should return a truncated string if input is over twenty characters`, () => {
      expect(transformImportedStringWithLimit(`Johnjacobjingleheimer`, `/filers/*/firstName`)).toBe(
        `Johnjacobjingleheime`
      );
      expect(transformImportedStringWithLimit(`Kleinschmidtingtonheimer`, `/filers/*/lastName`)).toBe(
        `Kleinschmidtingtonhe`
      );
    });
  });
